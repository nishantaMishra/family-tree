...
<script>
    ...
    function getVisibleLeafCount(node) {{
        if (!node || !node.children || node.children.length === 0) {{
            return 1;
        }}

        return node.children.reduce(function(total, child) {{
            return total + getVisibleLeafCount(child);
        }}, 0);
    }}
    ...
    function applySize() {{
        ...
        var visibleLeafCount = root ? getVisibleLeafCount(root) : 1;
        var minimumTreeHeight = visibleLeafCount * layout.rowSpacing;

        treeLayout.separation(function(a, b) {{
            return a.parent === b.parent ? 1 : 1.15;
        }});
        treeLayout.size([
            Math.max(120, s.h - margin.top - margin.bottom, minimumTreeHeight),
